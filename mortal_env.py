...
print('ENV: ', value_INFL, value_FORA, value_MU, value_SOS, value_POPMAX, value_SOL, flush=True)
#
# print(value1, ", ", value2)
...
const_min['mu'] = 1
const_max['mu'] = 20
#const_min['self_infl'] = const_self_min
#const_max['self_infl'] = const_self_max
...
class population:
    ...
    def __init__(self, param_time_to_live = const_time_to_live,
                 #param_genome_length = const_genome_length,
                 #param_alphabet = const_alphabet,
                 param_speed_of_speed_change = const_speed_of_speed_change,
                 #param_max_pop_size = const_max_pop_size,
                 #param_sample_size = const_sample_size,
    ...
    def stop_experiment_conditions(self):
        ...
        #print([org.abund for org in self.orgs])

        if tot_abund > const_max_pop_size:
            #print('BIOSPHERE IS TOO LARGE!!!!!')
            return 'Too large'

        if len(self.orgs) > self.param_biosphere_lim:
            #print('THERE ARE TOO MANY SPECIES IN BIOSPHERE!!!!!')
            return 'Too many species'

        if len(self.orgs) == 0:
            #print('BIOSPHERE CEASED!!!!!')
            return 'Ceased'

        if (time.time()-start_time)/60 > stop_time:
            f = open(r'./input_files/' + str(value_PATH) + '/env_' + str(round(value_FORA, 3)) + '_' + str(value_NUM) + '.txt', 'wb')
            pickle.dump(popul, f)
            f.close()
            exit()
            return 'Time out'
    ...
    def logs(self):
        ...
        if len(a)>0:
            self.loghash['abundance'].append(sum(a)) # общая численность биосферы
...
try:
  f = open('./data_out/' + str(value_PATH) + '/env_' + str(round(value_FORA, 3)) + '_' + str(value_NUM) + '.txt', 'wb')
