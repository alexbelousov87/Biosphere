...
print('MU: ', value_INFL, value_FORA, value_MU, value_SOS, value_POPMAX, value_SOL, flush=True)
...
class population:
    ...
    def stop_experiment_conditions(self):
        ...
        if (time.time()-start_time)/60 > stop_time:
            f = open(r'./input_files/' + str(value_PATH) + '/mu_' + str(value_MU) + '_' + str(value_NUM) + '.txt', 'wb')
            pickle.dump(popul, f)
            f.close()
            exit()
            return 'Time out'
...
try:
  f = open('./data_out/' + str(value_PATH) + '/mu_' + str(value_MU) + '_' + str(value_NUM) + '.txt', 'wb')
