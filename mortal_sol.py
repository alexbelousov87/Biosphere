...
print('SOL: ', value_INFL, value_FORA, value_MU, value_SOS, value_POPMAX, value_SOL, flush=True)
...
class population:
    ...
    def stop_experiment_conditions(self):
        ...
        if (time.time()-start_time)/60 > stop_time:
            f = open(r'./input_files/' + str(value_PATH) + '/sol_' + str(value_SOL) + '_' + str(value_NUM) + '.txt', 'wb')
            pickle.dump(popul, f)
            f.close()
            exit()
            return 'Time out'
    ...
    def __init__(self, popul=None, parent=None):
        ...
        # если это чей-то потомок, то наследуем с мутациями
            ...
            if popul.A[parent.index] >= const_born_threshold:
                ...
                # убавляем прибавку численности на последнем шаге, из которой данный вид мог бы породить мутантные виды
                #parent.abund_diff -= popul.param_initial_abundance
...
try:
  f = open('./data_out/' + str(value_PATH) + '/sol_' + str(value_SOL) + '_' + str(value_NUM) + '.txt', 'wb')
